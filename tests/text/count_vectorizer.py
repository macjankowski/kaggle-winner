...
class TestCountVectorizer(unittest.TestCase):
    ...
    def test_count_vectorizer(self):
        ...
        pipe = LettersCountTransformer("f_27")
    ...
    def common_test_ngram(self, drop_feature):
        ngam_transformer = NGramsCountTransformer("f_27", drop_feature=drop_feature)
        d = {
            'col1': [2, 5],
            'f_27': pd.Series(["ABACABD", "ACDFBAA"], index=[2, 3])
        }
    ...
    def test_count_vectorizer_bigrams(self):
        ngam_transformer = NGramsCountTransformer("f_27", ngram_range=(1,2))
        d = {
            'col1': [2, 5],
            'f_27': pd.Series(["ABACABD", "ACDFBAA"], index=[2, 3])
        }
    ...
    def test_count_vectorizer_binary(self):
        ngam_transformer = NGramsCountTransformer("f_27", binary=True)
        d = {
            'col1': [2, 5],
            'f_27': pd.Series(["ABACABD", "ACDFBAA"], index=[2, 3])
        }
